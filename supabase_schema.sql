...
CREATE POLICY "Categories manageable by admin" ON public.categories ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);

-- products
CREATE POLICY "Products readable by anyone" ON public.products FOR SELECT USING (true);
CREATE POLICY "Products manageable by producer owner" ON public.products ALL USING (
  auth.uid() = producer_id
);

-- producer_locations
CREATE POLICY "Locations readable by anyone" ON public.producer_locations FOR SELECT USING (true);
CREATE POLICY "Locations manageable by producer owner" ON public.producer_locations ALL USING (
  auth.uid() = producer_id
);

-- markets
CREATE POLICY "Markets readable by anyone" ON public.markets FOR SELECT USING (true);
CREATE POLICY "Markets manageable by admin" ON public.markets ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);
...
CREATE POLICY "Blog posts manageable by admin" ON public.blog_posts ALL USING (
  EXISTS (SELECT 1 FROM public.profiles WHERE id = auth.uid() AND role = 'admin')
);
